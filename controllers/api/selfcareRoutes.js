...
router.delete("/:id", withAuth, async (req, res) => {
  try {
    ...
    if (!selfCarePostData) {
      res
        .status(404)
        .json({ message: "No self care post found with this id!" });
      return;
    }
...
router.get("/search", (req, res) => {
  ...
  SelfCarePost.findAll({ where: { title: { [Op.like]: "%" + term + "% " } } })
    .then((selfCarePost) => res.render("Self-care", { selfCarePost }))
